...
@swagger_auto_schema(
    ...
    responses={
        200: openapi.Response(
            ...
            examples={"application/json": {"mensaje": "1) K5 xxx xxx xxx"}},
        ),
...
def getprediales(request: Request):
    try:
        ...
        return Response(
            {"mensaje": "Ha ocurrido un error. Intenta de nuevo más tarde."},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    responses={
        200: openapi.Response(
            ...
            examples={
                "application/json": {
                    "mensaje": (
                        "http://localhost:8000/media/predial-xxxxxxx.pdf"
                        if settings.DEBUG
                        else f"{os.environ.get('IP_SERVER')}/media/predial-xxxxxxx.pdf"
                    )
                }
            },
        ),
...
def getpredialpdf(request: Request):
    try:
        ...
        if predialObj is None:
            return Response(
                {
                    "mensaje": "Parece ser que no haz consultado con anterioridad los prediales. Te invitamos a que lo hagas."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if predialOption is None or not predialOption.isnumeric():
            return Response(
                {
                    "mensaje": "La opcion no es correcta. Ingresa una de las opciones indicadas anteriormente."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        PredialUrlShort.objects.create(
            original_url=shortUrl[2], short_id=shortUrl[0]
        ).save()

        accessLink = (
            f"http://localhost:8000/media/{filename}"
            if settings.DEBUG
            else f"{os.environ.get('IP_SERVER')}/media/{filename}"
        )
    ...
    responses={
        200: openapi.Response(
            ...
            examples={
                "application/json": {
                    "mensaje": (
                        "http://localhost:8000/tns-predial-pago-online/7882x8"
                        if settings.DEBUG
                        else f"{os.environ.get('IP_SERVER')}/tns-predial-pago-online/7882x8"
                    )
                }
            },
        ),
...
def redirectToOriginalLink(request: Request, short_id: str):
    try:
        ...
        return Response(
            {"mensaje": "El link al que intenta acceder no se encuentra disponible."},
