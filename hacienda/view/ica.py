...
def getlocalesbycc(request: Request):
    try:
        ...
        if cc is None:
            return Response(
                {
                    "mensaje": "El número de documento es requerido para realizar la consulta."
                },
...
def gethistorialbyplaca(request: Request):
    try:
        ...
        if placa is None:
            return Response(
                {"mensaje": "La placa es requerida para realizar la consulta."},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if not placa.isnumeric():
            return Response(
                {"mensaje": "La placa debe ser un número."},
                status=status.HTTP_400_BAD_REQUEST,
            )
